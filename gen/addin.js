...
const dataRange = "A1:I100"; // Define the range where data will be written
const rawData = [
...
function handleWorksheetChange(event) {
  if (event && event.address) {
    loadFromExcel(dataRange).then(data => {
      try {
        drawChart(data);
      } catch (error) {
        setStatus("Error drawing chart: " + error.message + JSON.stringify(data));
      }
    }).catch(error => {
...
Office.onReady(() => {
  ...
  writeToExcel(dataRange, rawData);
